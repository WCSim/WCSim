...
void subevents(char *filename=NULL) {
   ...
   * $ root -l -x 'subevents.C("../wcsim_momentum_electron_10.root")'
   */
  ...
  cout << "Number of subevents" << wcsimrootevent->GetNumberofsSubEvents() << endl;
  cout << "Number of Events" << wcsimrootevent-->GetNumberofEvents() << endl;
  ...
  for (int i = 0; i<max; i++){
    ...
    HvM->Fill(track->GetP(),track->GetM());
