...
#pragma GCC diagnostic pop
        ...
        // Diego Costas (diego.costas.rodriguez@usc.es) 2023
...
#pragma GCC diagnostic pop
    ...
    else if (useLightInjectorEvt)
    {
        ...
        if ( !LIGen ) {
            ...
            LIGen->ReadFromDatabase(injectorType,injectorIdx,injectorFilename,injectorDetails,injectorProfileFormat,injectorDetector,injectorWavelength,injectorPulseWidth);
        }
    ...
    else if (useMPMTledEvt)
    {
        ...
        for (HepMC3::GenParticlePtr part : hepmc3_reader->event.particles()) {
            ...
            if (part->status() == 1) {
                ...
                std::cout << "NuHepMC3Reader: [INFO] Particle ID: "
                          << part->pdg_id()
                          << "\n                       Status: "
                          << part->status()
                          << "\n                       Momentum: "
                          ...
                          << "\n                       Time: " << part->production_vertex()->position().t()
                          << "\n                       Direction: "
                          << part->momentum().px() << " " << part->momentum().py() << " " << part->momentum().pz()
                          << "\n                       Momentum mag: "
...
void WCSimPrimaryGeneratorAction::SetupBranchAddresses(ND::NRooTrackerVtx* nrootrackervtx){
    ...
    if (fHaveNFBranches)
    {
        ...
        fRooTrackerTree->SetBranchAddress("NFfirststep", (nrootrackervtx->NFfirststepTEMP));
