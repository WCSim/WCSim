...
void printTreeSummary(TTree *t, bool quiet)
{
  if(!quiet)
    cout << "The TTree \"" << t->GetName() << "\" takes " << sizeOnDisk(t) << " bytes on disk\n";
  else
    cout << "TREESIZE " << sizeOnDisk(t) << endl;
...
int printSizes(const char * filename, const char * treename, bool quiet)
{
