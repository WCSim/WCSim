...
WCSimPrimaryGeneratorMessenger::WCSimPrimaryGeneratorMessenger(
    WCSimPrimaryGeneratorAction *pointerToAction)
  ...
  genCmd->SetGuidance(" Available generators : muline, gun, laser, gps, "
                      "datatable, cosmics, radioactive, "
                      "rootracker, radon, injector, gamma-conversion");
  genCmd->SetParameterName("generator", true);
  genCmd->SetDefaultValue("muline");
  genCmd->SetCandidates(
      "muline gun laser gps datatable cosmics radioactive rootracker "
  ...
  poisCmd->SetGuidance(
      "Flag for generating photoelectrons directly on PMTs according to a "
  ...
  poisMeanCmd->SetGuidance(
      "Set Poisson mean to be used with /mygen/pmtPoisson. Defaults to 1.");
  ...
  radioactive_time_window_Cmd =
      new G4UIcmdWithADouble("/mygen/radioactive_time_window", this);
  radioactive_time_window_Cmd->SetGuidance(
      "Select time window for radioactivity");
  radioactive_time_window_Cmd->SetParameterName("radioactive_time_window",
                                                true);
  ...
  openingAngleCmd =
      new G4UIcmdWithADouble("/mygen/injector_opening_angle", this);
  ...
  injectorWavelengthCmd =
      new G4UIcmdWithADouble("/mygen/injector_wavelength", this);
...
void WCSimPrimaryGeneratorMessenger::SetNewValue(G4UIcommand *command,
  ...
  if (command == genCmd) {
                                          ...
                                          // Radioactivity (from F. Nova code)
    {
      ...
      myAction->SetRadonEvtGenerator(false);
               ...
               "radon") // G. Pronost: Addition of Radon generator (based on F.
  ...
  if (command == poisMeanCmd) {
    ...
    G4cout << "PoissonPMT mean set to: "
           << poisMeanCmd->GetNewDoubleValue(newValue) << G4endl;
  }
  ...
  if (command == injectorTimeCmd) {
    myAction->SetInjectorTimeWindow(
        injectorTimeCmd->GetNewDoubleValue(newValue));
  }

  if (command == openingAngleCmd) {
    myAction->SetInjectorOpeningAngle(
        openingAngleCmd->GetNewDoubleValue(newValue));
  }

  if (command == injectorWavelengthCmd) {
    myAction->SetInjectorWavelength(
        injectorWavelengthCmd->GetNewDoubleValue(newValue));
  }
}
...
void WCSimPrimaryGeneratorMessenger::RadonScalingCommand(G4String newValue) {
  ...
  if (scenario == "A")
    iScenario =
        1; // Relative scaling with respect to full ID volume (Pessimistic)
  if (scenario == "B")
