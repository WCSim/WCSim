  ...
  */
  ...
  for (int ev=0; ev<nevent; ev++)
  {
	...
	for (int j = timeArrayIndex; j < timeArrayIndex + peForTube; j++)
	{
	  ...
	  TObject *element = (wcsimrootevent->GetCherenkovHitTimes())->At(i);
    ...
    for (int index = 0 ; index < wcsimrootsuperevent->GetNumberOfEvents(); index++) 
    {
      ...
      h2nhits->Fill(ncherenkovdigihits, wcsimrootevent->GetTriggerInfo());
      h1nhitstrigger->Fill(wcsimrootevent->GetTriggerInfo());
      h1nhits->Fill(ncherenkovdigihits);

      if(verbose) cout << "Passed trigger "
		    // << /*EnumAsString*/(wcsimrootevent->GetTriggerType())
		       << " with timestamp "
		    // << wcsimrootevent->GetTriggerTime()
		       << " (c.f. " << trigger_time
		       << ") and " << ncherenkovdigihits
		       << " hits in the saved subevent region (" 
		    // << wcsimrootevent->GetTriggerInfo()
