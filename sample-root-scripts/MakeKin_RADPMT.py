...
# Open a file, read lines in
pmtnum_list=[]
x_list=[]
y_list=[]
z_list=[]
lines = [line.strip() for line in open("HKPMT_LIST.list", 'r')]
for number in lines:
    pmtnum,x,y,z =  number.split(" ")
    pmtnum_list.append(int(pmtnum))
...
parser.add_option("-v", "--vertex", dest="vertname",
                  help="Type of vertex (center*, random, randompmt, minusx, plusx, minusz, plusz)",
                  default="center")
    ...
    if randvert:
        ...
        #HK - 60x74m
            ...
            while True:
                ...
                f.write("$ info 0 0 %i\n" % recno)
        elif randvertpmt == True:
            pmt = random.randint(0,numpmt-1)
            #while True:
            x = x_list[pmt]
            y = y_list[pmt]
            z = z_list[pmt]
...
for fileno in range(nfiles):
    ...
    filename="PMT_%s_%.0fMeV_%s_%s_%03i.kin" % (typestr, energy, options.vertname, options.dirname, fileno)
