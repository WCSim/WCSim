...
//           - type of multiPMT object: sphere vs cylinder
//           - WinstonCone: TODO
							      ...
							      G4double expose)
{
  ...
  //Option 1: spherical orientation of single PMTs: KM3Net-style
  //Option 2: cylindrical orientation: PINGU/Mica-style
  
  G4LogicalVolume* logicWCMultiPMT; 
  int multi_orient = 1; //make into enum (0= cylindrical with spherical caps, 1 = spherical)
  if(multi_orient == 0){
    ...
    double multiRadius = 17*25.6*mm + expose; //17inch 

    G4Sphere* solidMultiPMT =
      new G4Sphere(    "WCMultiPMT",
                       0.0*m,radius,
                       0.0*deg,360.0*deg,
                       0.0*deg,180.0*deg);
    ...
    G4ThreeVector pos_offset[nPMTs] = {G4ThreeVector(0,0,0),
				       G4ThreeVector(0,0,0)}; //make this more user friendly/defineable later
    std::pair<G4double,G4double> orientation[nPMTs] = {std::pair<G4double, G4double>(90.*deg,0.*deg),
						       std::pair<G4double, G4double>(90.*deg,180.*deg)}; //theta,phi
