...
void plot_tree(int ievent = 0, int itrigger = 0)
{
  ...
  figname=Form("figure/display_event_%d_trigger_%d.eps",ievent,itrigger);
  ...
  if( make_animation ){
	...
	if( the_time < _time_min || the_time > _time_max ) continue; //select relevant time window
      ...
      figname=Form("figure/display_event_%d_trigger_%d_time_%s.gif",ievent,itrigger,number_with_digits(i,n_digits).c_str());
      c_display.Print(figname.Data());
