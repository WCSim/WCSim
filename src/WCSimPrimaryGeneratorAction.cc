        ...
        if (useNuanceTextFormat) {
            ...
            if (token.size() == 0 || token[0] == "stop") {
                ...
                while (token[0] == "nuance" && iVertex < MAX_N_VERTICES) {
                    ...
                    // B.Q: there can be some cases (2p2h i.e. neut mode = 2) where there are 2 targets. The while
                    // loop is added for this purpose.
...
#pragma GCC diagnostic pop
        ...
        // The current neut vector files do not correspond directly to the detector dimensions, so only keep those
        ...
        particleGun->SetParticlePosition(vtx);
        particleGun->SetParticleMomentumDirection(dir);
        // Will want to include some beam time structure at some point, but not needed at the moment since we only
        // simulate 1 interaction per events particleGun->SetParticleTime(time);
...
#pragma GCC diagnostic pop
    ...
    } else if (useInjectorEvt) // K.M.Tsui: addition of injector events
    {
        ...
        while (std::getline(inputFile, line)) {
            ...
            // We've reached the end of the event N that we're generating and have also read the first line of
            // event N + 1. We need to rewind to the start of event N + 1 so that the first particle of N + 1 is
            // not missed when this function is called again for the next event.
        ...
        for (it = radioactive_sources.begin(); it != radioactive_sources.end(); it++) {
            ...
            for (int u = 0; u < n_vertices; u++) {
                ...
                if (IsotopeLocation.compareTo("water") == 0) {
                    ...
                    // random_R " << random_R << " orientation (" << orientation.x() << ", " << orientation.y() <<
                    // ", " << orientation.z() << ") center (" << random_pmt_center.x() << ", " <<
                    // random_pmt_center.y() << ", " << random_pmt_center.z() << ") position (" << position.x() <<
                    // ", " << position.y() << ", " << position.z() << ") " << G4endl;
        ...
        G4int centerPMTId = 0;
        G4int firstPMTId = 0;
        static std::map<int, std::pair<int, int>> mpmt_id_map =
            usePMT1 ? myDetector->GetTube_mPMTIDMap() : myDetector->GetTube_mPMTIDMap2();
        for (G4int i = 1; i <= nID_mPMTs; i++) {
            if (mpmt_id_map[i].first == mPMTLEDId1) {
                if (mpmt_id_map[i].second == 1)
                    centerPMTId = i;
                else if (mpmt_id_map[i].second == 2)
                    firstPMTId = i;
            }
        }
        ...
        G4Transform3D tubeTransformCenter =
            usePMT1 ? myDetector->GetTubeTransform(centerPMTId) : myDetector->GetTubeTransform2(centerPMTId);
        G4Transform3D tubeTransformFirst =
            usePMT1 ? myDetector->GetTubeTransform(firstPMTId) : myDetector->GetTubeTransform2(firstPMTId);
        ...
        G4Vector3D posFirst(tubeTransformFirst.getTranslation().getX(), tubeTransformFirst.getTranslation().getY(),
                            tubeTransformFirst.getTranslation().getZ());
        G4double distFromOriginToPMT =
            (posCenter - posFirst).mag() / (pmtOrientationCenter - pmtOrientationFirst).mag();
        ...
        LEDdir = sin(LEDth + mPMTLED_dTheta) *
                     (cos(LEDphi + mPMTLED_dPhi) * x_axis + sin(LEDphi + mPMTLED_dPhi) * y_axis) +
                 cos(LEDth + mPMTLED_dTheta) * z_axis;
        ...
        for (size_t part_i = 0; part_i < event.particles().size(); part_i++) {
            ...
            if (event.particles()[part_i]->status() == 4) {
                ...
                G4cout << "Particle " << part_i << " has PDG " << event.particles()[part_i]->pid()
                       << " and momentum " << momentum.px() << ", " << momentum.py() << ", " << momentum.pz()
                       << ", " << momentum.e() << " and status " << event.particles()[part_i]->status() << G4endl;
                hepmc3_reader->RotateEvent();
            }
            momentum = hepmc3_reader->GetLorentzVector(part_i);
            if (event.particles()[part_i]->status() == 4) {
                G4cout << G4endl << "After rotation" << G4endl;
                G4cout << "Particle " << part_i << " has PDG " << event.particles()[part_i]->pid()
                       << " and momentum " << momentum.px() << ", " << momentum.py() << ", " << momentum.pz()
                       << ", " << momentum.e() << " and status " << event.particles()[part_i]->status() << G4endl;
            }
...
void WCSimPrimaryGeneratorAction::OpenRootrackerFile(G4String fileName) {
    ...
    if (!fRooTrackerTree) {
        G4cout << "File: " << fileName
               << " does not contain a Rootracker nRooTracker tree - please check you intend to process "
                  "Rootracker events"
               << G4endl;
        exit(1);
    }
