...
public:
  ...
  inline void SetRootrackerEvtGenerator(G4bool choice) {
    useRootrackerEvt = choice;
  }
  ...
  inline void SetInjectorEvtGenerator(G4bool choice) {
    useInjectorEvt = choice;
  }
  ...
  inline void OpenCosmicsFile(G4String fileName) {
    ...
    if (!inputCosmicsFile.is_open()) {
      G4cout << "Cosmics data file " << cosmicsFileName << " not found"
             << G4endl;
      exit(-1);
    }
  }
  ...
  inline std::vector<struct radioactive_source> Radioactive_Sources() {
    return radioactive_sources;
  }

  inline void SetRadioactiveEvtGenerator(G4bool choice) {
    useRadioactiveEvt = choice;
  }
  inline G4bool IsUsingRadioactiveEvtGenerator() { return useRadioactiveEvt; }

  inline void SetRadioactiveTimeWindow(G4double choice) {
    radioactive_time_window = choice;
  }
  ...
  inline void SetConversionProductParticle(int i,
                                           const G4ParticleDefinition *p) {
    conversionProductParticle[i] = p;
  }
  inline void SetConversionProductMomentum(int i, const G4ThreeVector &p) {
    conversionProductMomentum[i] = p;
  }
